...
switch ($action) {
    case 'video_open_list' :

        // 보관기간이 지난 상품 삭제
        
        $vi_list = explode(",", $vi_lst);

        $i = 0;

        for($i = 0; $i < count($vi_list); $i++)
        {
            $vi_id = trim($vi_list[$i]);
            if($vi_id != "")
            {
                $sql = " update g5_video set vi_open = '".$vi_open."' where vi_id = '$vi_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);
            }
        }
    ...
    case 'biz_open_list' :
        ...
        for($i = 0; $i < count($vi_list); $i++)
        {
            $wr_id = trim($vi_list[$i]);
            if($wr_id != "")
            {
                $sql = " update g5_write_biz_news set wr_1 = '".$wr_1."' where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);
            }
        }
    ...
    case 'event_open_list' :
        ...
        for($i = 0; $i < count($vi_list); $i++)
        {
            ...
            if($wr_id != "")
            {
                $sql = " update g5_write_event_info set wr_1 = '".$wr_1."' where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);
            }
        }
    ...
    case 'biz_del_list' :
        ...
        for($i = 0; $i < count($vi_list); $i++)
        {
            ...
            if($wr_id != "")
            {
                $row = sql_fetch("select * from g5_write_biz_news where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."'");
                ...
                // 업로드된 파일이 있다면 파일삭제
                $sql2 = " select * from {$g5['board_file_table']} where bo_table = 'biz_news' and wr_id = '{$wr_id}' ";
                $result2 = sql_query($sql2);
                while ($row2 = sql_fetch_array($result2)) {

                    $delete_file = run_replace('delete_file_path', G5_DATA_PATH.'/file/biz_news/'.str_replace('../', '', $row2['bf_file']), $row2);
                    if( file_exists($delete_file) ){
                        @unlink($delete_file);
                    }
                    // 썸네일삭제
                    if(preg_match("/\.({$config['cf_image_extension']})$/i", $row2['bf_file'])) {
                        delete_board_thumbnail("biz_news", $row2['bf_file']);
                    }
                }
                ...
                // 파일테이블 행 삭제
                sql_query(" delete from {$g5['board_file_table']} where bo_table = 'biz_news' and wr_id = '{$wr_id}' ");

                $sql = " delete from g5_write_biz_news where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);

                sql_query(" delete from {$g5['board_new_table']} where bo_table = 'biz_news' and wr_parent = '{$wr_id}' ");
        ...
        if ($count_write > 0)
        {
            sql_query(" update {$g5['board_table']} set bo_count_write = bo_count_write - '$count_write' where bo_table = 'biz_news' ");
        }
    ...
    case 'event_del_list' :
        ...
        for($i = 0; $i < count($vi_list); $i++)
        {
            ...
            if($wr_id != "")
            {
                $row = sql_fetch("select * from g5_write_event_info where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."'");
                ...
                // 업로드된 파일이 있다면 파일삭제
                $sql2 = " select * from {$g5['board_file_table']} where bo_table = 'event_info' and wr_id = '{$wr_id}' ";
                $result2 = sql_query($sql2);
                while ($row2 = sql_fetch_array($result2)) {

                    $delete_file = run_replace('delete_file_path', G5_DATA_PATH.'/file/event_info/'.str_replace('../', '', $row2['bf_file']), $row2);
                    if( file_exists($delete_file) ){
                        @unlink($delete_file);
                    }
                    // 썸네일삭제
                    if(preg_match("/\.({$config['cf_image_extension']})$/i", $row2['bf_file'])) {
                        delete_board_thumbnail("event_info", $row2['bf_file']);
                    }
                }
                ...
                // 파일테이블 행 삭제
                sql_query(" delete from {$g5['board_file_table']} where bo_table = 'event_info' and wr_id = '{$wr_id}' ");

                $sql = " delete from g5_write_event_info where wr_id = '$wr_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);

                sql_query(" delete from {$g5['board_new_table']} where bo_table = 'event_info' and wr_parent = '{$wr_id}' ");
        ...
        if ($count_write > 0)
        {
            sql_query(" update {$g5['board_table']} set bo_count_write = bo_count_write - '$count_write' where bo_table = 'event_info' ");
        }
    ...
    case 'video_del_list' :
        ...
        for($i = 0; $i < count($vi_list); $i++)
        {
            ...
            if($vi_id != "")
            {
                $sql = " delete from g5_video where vi_id = '$vi_id' and mb_id = '".$member["mb_id"]."' ";
                sql_query($sql);
            }
        }
