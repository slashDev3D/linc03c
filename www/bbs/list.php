...
if($bo_table == "event" && $ing != "")
{
    $is_search_bbs = true;      //검색구분변수 true 지정

    if($ing == "n")
    {
        $sql_search = " (wr_1 >= STR_TO_DATE(NOW(), '%Y-%m-%d')) ";
    }
    else if($ing == "t")
    {
        $sql_search = " (wr_1 <= STR_TO_DATE(NOW(), '%Y-%m-%d') and wr_2 >= STR_TO_DATE(NOW(), '%Y-%m-%d')) ";
    }else if($ing == "f")
    {
        $sql_search = " (wr_2 < STR_TO_DATE(NOW(), '%Y-%m-%d')) ";
    }
...
if($page_rows > 0) {
    ...
    while ($row = sql_fetch_array($result))
    {
        ...
        $str_target1 = strtotime($list[$i]["wr_1"]);
        $str_target2 = strtotime($list[$i]["wr_2"]);

        if($str_target1 >= $str_now)
        {
            $list[$i]["ing"] = "n";
        }
        else if($str_target1 <= $str_now && $str_target2 >= $str_now)
        {
            $list[$i]["ing"] = "t";
        }else if($str_target2 < $str_now)
        {
            $list[$i]["ing"] = "f";
        }
...
if(!$is_admin && $bo_table == "event")
{
    include_once($board_skin_path.'/list_user.skin.php');
}
else{
