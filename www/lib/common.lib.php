...
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="", $css="board")
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#(&amp;)?page=[0-9]*#', '', $url);
	$url .= substr($url, -1) === '?' ? 'page=' : '&amp;page=';
    ...
    if ($cur_page > 1) {
        //$str .= '<a href="'.$url.'1'.$add.'" class="pg_page pg_start">처음</a>'.PHP_EOL;
        $str .= '<a href="'.$url.'1'.$add.'"><div class="'.$css.'--index-button"><span class="material-symbols-outlined">keyboard_double_arrow_left</span></div></a>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'"><div class="'.$css.'--index-button"><span class="material-symbols-outlined">keyboard_arrow_left</span></div></a>'.PHP_EOL;

    if ($total_page > 0) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<a href="'.$url.$k.$add.'"><div class="'.$css.'--index-button"><span>'.$k.'</span></div></a>'.PHP_EOL;
            else
                $str .= '<div class="'.$css.'--index-button on"><span>'.$k.'</span></div>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'"><div class="'.$css.'--index-button"><span class="material-symbols-outlined">keyboard_arrow_right</span></div></a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="'.$url.$total_page.$add.'"><div class="'.$css.'--index-button"><span class="material-symbols-outlined">keyboard_double_arrow_right</span></div></a>'.PHP_EOL;
    }

    if ($str)
        return "<div class=\"".$css."--index\"><div class=\"".$css."--index-wrap\">{$str}</div></div>";
    else
        return "";
}
...
function get_board_date($date)
{
    $rtn = "";
    $rtn = date('Y.m.d', strtotime($date));
    return $rtn;
}

function get_board_view_date($date)
{
    $rtn = "";
    ...
    $date2 = $yoil[date('w', strtotime($date))];
    $date3 = date('H:i', strtotime($date));

    //$rtn = $date1." (".$date2."요일) ".$date3;
    $rtn = $date1." ".$date3;
    return $rtn;
}