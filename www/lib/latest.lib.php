...
function latest_linc($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='', $ca_name='')
{
    global $g5;
    ...
    if( $caches === false ){
        ...
        if($bo_table == "calender")
        {
            $sql = " select * from {$tmp_write_table} where ca_name = '".$ca_name."' and wr_3 != '' and wr_is_comment = 0 order by wr_1 desc limit 0, {$rows} ";
        }
        else if($bo_table == "event")
        {
            $sql = " select * from {$tmp_write_table} where (wr_2 >= STR_TO_DATE(NOW(), '%Y-%m-%d')) and wr_is_comment = 0 order by wr_2 limit 0, {$rows} ";
        }
        else{
            $sql = " select * from {$tmp_write_table} where ca_name = '".$ca_name."' and wr_3 != '' and wr_is_comment = 0 order by wr_num limit 0, {$rows} ";
        }
